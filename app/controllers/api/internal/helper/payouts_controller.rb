...
class Api::Internal::Helper::PayoutsController < Api::Internal::Helper::BaseController
  ...
  def index
    ...
    render json: { success: true, last_payouts: payouts, next_payout_date:, balance_for_next_payout: }
  end
  ...
  def create
    ...
    if Payouts.is_user_payable(@user, payout_date, processor_type: payout_processor_type, add_comment: true, from_admin: true)
      ...
      render json: { success: false, message: "User is not eligible for payout." }, status: :unprocessable_entity
    end
  end
